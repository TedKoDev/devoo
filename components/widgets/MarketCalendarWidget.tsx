  ...
  return (
    <div className="widget-card">
      <div className="flex justify-between items-center mb-3">
        <h3 className="text-sm font-medium">증시 이슈 캘린더</h3>
        <div className="flex items-center space-x-2">
          <button
            onClick={() => setViewMode("list")}
            className={`p-1 rounded-md ${viewMode === "list" ? "bg-primary text-white" : "text-gray-500 hover:bg-gray-100"}`}
          >
            <List className="h-3 w-3" />
          </button>
          <button
            onClick={() => setViewMode("calendar")}
            className={`p-1 rounded-md ${viewMode === "calendar" ? "bg-primary text-white" : "text-gray-500 hover:bg-gray-100"}`}
          >
            ...
            {dates.map((date) => (
              <button
                ...
                className={`px-2 py-1 text-xs rounded-md whitespace-nowrap ${
                  selectedDate === date ? "bg-primary text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                }`}
              >
                {formatDate(date)}
              </button>
              ...
              selectedEvents.map((event) => (
                <div key={event.id} className="p-2 hover:bg-gray-50 rounded-md -mx-2">
                  <div className="flex justify-between items-start">
                    ...
                    <div className={`text-xs px-1.5 py-0.5 rounded ${getImportanceBadgeStyle(event.importance)}`}>
        ...
        <div className="mt-2">
          ...
          <div className="mt-3 space-y-2">
            ...
            {selectedEvents.length > 2 && (
              <div className="text-xs text-right text-primary">+{selectedEvents.length - 2}개 더보기</div>
            )}
          </div>
        </div>
      )}
